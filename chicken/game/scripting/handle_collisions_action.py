...
from game.casting.fox import Fox
...
class HandleCollisionsAction(Action):
    ...
    def _handle_collision(self, cast):
        ...
        fox_rows = cast.get_actors("fox")
        fox_base_row = fox_rows[0]
        
        self.check_fox_collision(fox_base_row, cast, 50)
    ...
    def check_log_collision(self, row, cast, y):   
        """works on colision between the chicken and the log on a particular y axis, which is also the same as the position of the logs in that lane
    ...
    def check_fox_collision(self, row, cast, y):  
        """works on colision between the chicken and the fox on a particule y axis, which is also the same as the position of the fox in that lane

        Args:
            row (int): the row/lane of the fox
            cast (Cast): The cast of Actors in the game.
            y (int): y axis of the fox
        """ 
        chicken = cast.get_first_actor("chicken")         
        fox_list = row.get_foxes()
        
        for fox in fox_list:
            
            if chicken.get_position().get_y() == y and chicken.get_position().get_x() in range(fox.get_position().get_x() - 20, fox.get_position().get_x() + 40):
                self._is_game_over = True 
                animation = chicken.get_animation()
                animation.set_boom(True)
    ...
    def _restart(self, cast, script):
        ...
        if menu.restart_state():
            ...
            fox_rows = cast.get_actors("fox")
            for rows in fox_rows:
                rows.start_foxes(randint(1,1))
    ...
    def _handle_game_over(self, cast, script):
        """Adds the game over text to the menu screen, stops the cars, logs and foxes and sets _is_game_over to true
